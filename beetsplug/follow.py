...
def credentials_required(func):
    ...
    def wrapper(*args, **kwargs):
        password_is_unmanaged = password_mgr.find_user_password(None, MUSPY_API) == (
            None,
            None,
        )
...
class FollowPlugin(BeetsPlugin):
    ...
    def commands(self):
        follow_cmd = ui.Subcommand(
            "follow", help="Follow album artist of each item in query"
        )
        unfollow_cmd = ui.Subcommand(
            "unfollow", help="Unfollow album artist of each item in query"
        )
    ...
    def get_album_artists(self, items):
        ...
        return sorted(
            set(
                [
                    (album.get("mb_albumartistid"), album.albumartist)
                    for album in albums
